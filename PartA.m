...
% [m,n,p] = size(image);
% dest = round( [1 1; 1 n; m 1; m n]*S );
% dest = bsxfun(@minus, dest, min(dest)) + 1;
% imagescale = zeros([max(dest) p],class(image));
% 
%         for ii = 1:size(imagescale,1)
%             for jj = 1:size(imagescale,2)
%                 source = ([ii jj]-dest(1,:))*S.';
%                 if all(source >= 1) && all(source <= [m n])
% 
%                     % Get all 4 surrounding pixels
%                     C = ceil(source);
...
% 
%                     % Assign                     
%                     imagescale(ii,jj,:) = sum(sum(cols),2);
% 
%                 end
%             end
%         end 
