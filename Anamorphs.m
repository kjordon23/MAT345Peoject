...
% Rotation by area mapping. For each pixel, it finds 4 source pixels that
% it covers. Determines destination pixel on an weighted-average between
% those 4 pixels. 
image = imread('pic.png');

% Does not handle 90, 180, or 270 degrees (Special Cases Note Available in
% implementation.
...
imagerot = zeros([max(dest) p],class(image)); % Insert 0 Values in new Matrix
...
[nrows ncols nslices] = size(imagescale);
midx=ceil((ncols+1)/2);
midy=ceil((nrows+1)/2);
...
imagestretch = reshape(imagestretch,size(imagescale)); % Resizes the size of the matrix to the scale factor
...
dest = round( [1 1; 1 n; m 1; m n]*R );
dest = bsxfun(@minus, dest, min(dest)) + 1;
imagerot = zeros([max(dest) p],class(image)); % Insert Place Values in new Matrix
